...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const successUrl = `${SITE_URL}/dashboard/payment/success?session_id={CHECKOUT_SESSION_ID}&phase=final&project_id=${projectId}`;
    const cancelUrl = `${SITE_URL}/dashboard/payment/cancel?phase=final&project_id=${projectId}`;
    ...
    const response = await fetch("https://api.stripe.com/v1/checkout/sessions", {
      ...
      body: new URLSearchParams({
        ...
        "line_items[0][price_data][product_data][name]": "Płatność końcowa za obraz na zamówienie",
