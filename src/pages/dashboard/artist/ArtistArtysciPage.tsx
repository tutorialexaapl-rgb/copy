...
import { Search, SlidersHorizontal, X, ArrowDownWideNarrow, Sparkles, ArrowRight } from 'lucide-react';
...
export function ArtistArtysciPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description="Przeglądaj profile innych artystów, ich portfolio i style - znajdź inspirację i twórców o podobnej estetyce."
      />
      ...
      <div className="rounded-2xl border border-gold-400/20 bg-gradient-to-br from-gold-50 to-ivory-100 p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="font-display text-lg text-graphite-600">Odkryj twórców platformy</h3>
            <p className="mt-1 text-sm text-graphite-400">
              Zobacz, co tworzą inni artyści. Portfolio służy pokazaniu stylu i jakości prac - to nie jest sklep.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((a) => (
            <div key={a.id} className="space-y-3">
              <ArtistCard artist={a} to={`/dashboard/artist/artysci/${a.slug}`} />
              <ArtistCatalogCardActions artist={a} />
            </div>
          ))}
        </div>
      ) : (
...
function ArtistCatalogCardActions({ artist }: { artist: ArtistProfile }) {
  return (
    <Link
      to={`/dashboard/artist/artysci/${artist.slug}`}
